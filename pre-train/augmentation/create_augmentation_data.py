# This file is to create the datafile for supervised fine tuning task.
# The label returns 1 if the two firms are in the same GIC Sector. Otherwise the label returns 0.
...
for i in range(len(w)):
    for j in range(i+1,len(w),1):
        t.append([w[i], w[j], label(w[i],w[j])])

df = pd.DataFrame(t)
df.columns = ["#1 ID", "#2 ID", "Quality"]
...
with open('/Users/rachelzheng/Documents/GitHub/bert4search/data/train.tsv', 'wt') as out_file:
    tsv_writer = csv.writer(out_file, delimiter='\t')
    tsv_writer.writerow(['Quality', '#1 ID', '#2 ID', '#1 String', '#2 String'])
    for i in range(train.shape[0]):
        s = [str(train.iloc[i,2]), str(train.iloc[i,0]), str(train.iloc[i,1]), str(train.iloc[i,0]), str(train.iloc[i,1])]
        tsv_writer.writerow(s)

with open('/Users/rachelzheng/Documents/GitHub/bert4search/data/dev.tsv', 'wt') as out_file:
    tsv_writer = csv.writer(out_file, delimiter='\t')
    tsv_writer.writerow(['Quality', '#1 ID', '#2 ID', '#1 String', '#2 String'])
    for i in range(validate.shape[0]):
        s = [str(validate.iloc[i,2]), str(validate.iloc[i,0]), str(validate.iloc[i,1]), str(validate.iloc[i,0]), str(validate.iloc[i,1])]
        tsv_writer.writerow(s)

with open('/Users/rachelzheng/Documents/GitHub/bert4search/data/test.tsv', 'wt') as out_file:
    tsv_writer = csv.writer(out_file, delimiter='\t')
    tsv_writer.writerow(['Quality', '#1 ID', '#2 ID', '#1 String', '#2 String'])
    for i in range(test.shape[0]):
        s = [str(test.iloc[i,2]), str(test.iloc[i,0]), str(test.iloc[i,1]), str(test.iloc[i,0]), str(test.iloc[i,1])]
        tsv_writer.writerow(s)
